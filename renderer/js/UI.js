...
let char1 = new Character('Jacop', 'models/characters portraits/char1.jpeg', 133, 2, 6, 5)
...
function drawUI () {
  ...
  ctx.clearRect(0, 0, c.width, c.height)
      // draw bg image
  if (isBgLoaded) { ctx.drawImage(bgImg, 0, 0, 1280, 720) }
  ...
  ctx.stroke()
      ...
      // draw portrait image
  if (charcter.portraitImageReady) { ctx.drawImage(charcter.portraitImage, xposPortrait + 2, yposPortrait + 2, widthPortrait - 4, heightPortrait - 4) }
  ...
  for (var i = 0; i < 4; i++) {
    if (j >= charcter.inventory.length) { break }
    if (j == charcter.inventory.length) { console.log('it went too much') }
    var w = (widthItems / 4) - 12
    var h = heightItems - 20
...
function changeStat (stat, num) {
  ...
  if (num > 0) { charcter.messagesLog.push('You gained ' + num + ' ' + stat + '!') } else { charcter.messagesLog.push('You lost ' + (-1) * num + ' ' + stat + '!') }
}
